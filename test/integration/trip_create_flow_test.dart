...
void main() {
  group('T016: Integration - Create Trip Flow', () {
    ...
    test('should complete full trip creation flow with all side effects', () async {
      // Arrange
      ...
      verify(mockTripRepository.getAllTrips()).called(greaterThanOrEqualTo(1));

      // 5. Check state emissions
      expect(stateEmissions, [
        isA<TripCreating>(),
        isA<TripCreated>().having((s) => s.trip.id, 'trip.id', createdTrip.id),
        isA<TripLoading>(),
