...
void main() {
  ...
  group('T026: TripCubit.joinTrip adds participant and logs activity', () {
    test('should add user as participant and log activity when joining trip', () async {
      // Arrange
      ...
      final capturedTrip = verify(mockTripRepository.updateTrip(captureAny)).captured.single as Trip;
  ...
  group('T027: TripCubit.joinTrip is idempotent (already member)', () {
    test('should not add duplicate participant if user already a member', () async {
      // Arrange
      ...
      when(mockTripRepository.getAllTrips()).thenAnswer((_) => Stream.value([existingTrip]));
  ...
  group('T028: TripCubit.joinTrip handles trip not found', () {
    test('should emit error when trip does not exist', () async {
      ...
      when(mockTripRepository.getTripById(tripId)).thenAnswer((_) async => null);
  ...
  group('T029: TripCubit.isUserMemberOf checks local cache', () {
    test('should return true if trip ID is in joined trips cache', () {
      ...
      when(mockLocalStorageService.getJoinedTripIds()).thenReturn([tripId, 'trip-888']);
    ...
    test('should return false if trip ID is not in joined trips cache', () {
      // Arrange
      const tripId = 'trip-999';
      when(mockLocalStorageService.getJoinedTripIds()).thenReturn(['trip-888', 'trip-777']);
  ...
  group('hasDuplicateMember', () {
    test('returns true when participant with matching name exists (exact match)', () async {
      // Arrange
    ...
    test('returns true when participant exists with different casing (case-insensitive)', () async {
      // Arrange
      final now = DateTime.now();
      final trip = Trip(
        id: 'trip-1',
    ...
    test('returns false when no participant with matching name exists', () async {
      // Arrange
      final now = DateTime.now();
      final trip = Trip(
        id: 'trip-1',
